...
make_player <- function(
  ...
  list(
    ...
    position = if (length(position) != 1 || is.na(position)) {
      NA_character_
    } else {
...
make_advance <- function(
  ...
  list(
    ...
    to = as.integer(to),
    scored = isTRUE(scored),
    out = isTRUE(out),
    earned = isTRUE(earned)
  )
}
...
validate_event <- function(evt) {
  ...
  if (!evt$type %in% EVENT_TYPES) {
    add(paste("unknown event type:", evt$type))
  }
  if (identical(evt$type, "plate_appearance")) {
    o <- evt$payload$outcome
    if (is.null(o) || !o %in% c(APP_CONFIG$outcome_codes)) {
      add(paste("bad outcome:", o %||% "NULL"))
    }
    if (!evt$payload$team %in% c("home", "away")) {
      add("plate_appearance needs team home/away")
    }
  }
  if (identical(evt$type, "substitution")) {
    if (!evt$payload$kind %in% c("batting", "defensive", "courtesy_runner")) {
      add("bad sub kind")
    }
  }
  if (identical(evt$type, "half_runs")) {
    if (!isTRUE(evt$payload$team %in% c("home", "away"))) {
      add("half_runs needs team home/away")
    }
    r <- evt$payload$runs
    if (is.null(r) || !is.numeric(r) || r < 0) {
      add("half_runs needs non-negative runs")
    }
  }
