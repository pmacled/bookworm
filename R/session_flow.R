storage_for_identity <- function(
  ...
  if (!identical(identity$mode, "user")) {
    return(guest())
  }
  ...
  if (!driver_ok) {
    return(guest(
      TRUE,
      "The database driver is not available on this deployment. This game will not be saved."
    ))
  }
  ...
  if (inherits(con, "error")) {
    return(guest(
      TRUE,
      "Could not reach the database. This game will not be saved."
    ))
  }

  list(
    storage = make_storage("supabase", con = con, user_id = identity$user_id),
    con = con,
    degraded = FALSE,
    reason = ""
  )
