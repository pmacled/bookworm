...
.disposition_row_ui <- function(ns, row, selected) {
  label <- paste0(
    ...
    row$name,
    "  (",
    .BASE_LABEL[[row$from + 1L]],
    ")"
  )
  tags$div(
    class = "bw-disp-row d-flex align-items-center justify-content-between gap-2",
    tags$span(class = "bw-disp-name", label),
    radioButtons(
      ns(paste0("disp_", row$runner_id)),
      NULL,
...
disposition_modal_ui <- function(
  ...
  modalDialog(
    title = sprintf("Baserunners — %s", lbl),
    tags$p(
    ...
    tags$div(
      ...
      !!!lapply(rows, function(r) {
        .disposition_row_ui(ns, r, prefill[[r$runner_id]])
      })
    ),
    if (length(errors)) {
      tags$div(
...
read_disposition_choices <- function(input, rows) {
  ...
  for (r in rows) {
    ...
    out[[r$runner_id]] <- if (is.null(v) || length(v) != 1 || is.na(v)) {
      ""
    } else {
      as.character(v)
    }
