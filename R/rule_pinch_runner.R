...
evaluate_pinch_runner <- function(cfg, state, out_player, in_player) {
  ...
  n_inning <- sum(vapply(
    for_team,
    function(e) {
      identical(e$inning, state$inning) && identical(e$half, state$half)
    },
    logical(1)
  ))
  if (!is.na(pr$max_per_inning) && n_inning >= pr$max_per_inning) {
    add(
      "pr_max_per_inning",
      sprintf(
        "Only %d pinch runner(s) allowed per inning; %d already used.",
        pr$max_per_inning,
        n_inning
      )
    )
  ...
  if (!is.na(pr$max_per_game) && length(for_team) >= pr$max_per_game) {
    ...
    if (pr$max_per_game == 0L) {
      ...
      add(
        "pr_max_per_game",
        sprintf(
          "Only %d pinch runner(s) allowed per game; %d already used.",
          pr$max_per_game,
          length(for_team)
        )
      )
  ...
  n_player <- sum(vapply(
    for_team,
    function(e) identical(e$in_player_id, in_player$player_id),
    logical(1)
  ))
  if (
    !is.na(pr$max_per_player_per_game) && n_player >= pr$max_per_player_per_game
  ) {
    add(
      "pr_max_per_player",
      sprintf(
        "%s has already pinch run %d time(s) this game.",
        in_player$name,
        n_player
      )
    )
  ...
  if (
    identical(pr$allowed_for, "pitcher_catcher") &&
  ...
  elig <- pr$eligibility
  if (
    identical(elig, "same_gender") &&
      !identical(in_player$gender, out_player$gender)
  ) {
    add(
      "pr_same_gender",
  ...
  if (elig %in% c("last_out", "last_same_gender_out")) {
    ...
    if (identical(elig, "last_same_gender_out")) {
      ...
      outs <- Filter(
        function(id) identical(.gender_of(state, team, id), want),
        outs
      )
    }
    ...
    if (is.na(expected)) {
      ...
      add(
        "pr_last_out",
        sprintf(
          "The runner must be the last %sout (%s).",
