default_ruleset_config <- function() {
  list(
    ...
    batting_size_rule = "max",
    fielding = list(
      fielder_count = NA_integer_,
      min_females = 0L,
    ...
    home_run_rule = list(
      over_fence_limit = NA_integer_,
      limit_by_gender = list(),
      over_limit_result = "out",
      inside_park_counts = FALSE
    ),
    pinch_runner = list(
      max_per_inning = NA_integer_,
...
.migrate_ruleset_config <- function(cfg) {
  ...
  if (!is.null(m) && !is.null(m$differential)) {
    ...
    if (length(d) == 1 && !is.na(d)) {
      ...
      after <- if (is.null(after) || length(after) != 1 || is.na(after)) {
        1L
      } else {
        as.integer(after)
      }
  ...
  if (
    !is.null(bg) &&
      !is.null(bg$type) &&
      bg$type %in% names(.BATTING_GENDER_ALIASES)
  ) {
...
coerce_ruleset_config <- function(cfg) {
  ...
  if (!is.na(d$batting_size) && d$batting_size < 1L) {
    d$batting_size <- NA_integer_
  }
  d$batting_size_rule <- d$batting_size_rule %||% "max"
  if (!d$batting_size_rule %in% c("max", "exact")) {
    d$batting_size_rule <- "max"
  }
  ...
  d$mercy_rule$tiers <- lapply(d$mercy_rule$tiers %||% list(), function(t) {
    list(
      after_inning = .as_int_or_na(t$after_inning),
      differential = .as_int_or_na(t$differential)
    )
  })

  d$home_run_rule$over_fence_limit <- .as_int_or_na(
    d$home_run_rule$over_fence_limit
  )
  d$home_run_rule$limit_by_gender <-
    lapply(d$home_run_rule$limit_by_gender %||% list(), .as_int_or_na)
  d$home_run_rule$inside_park_counts <- isTRUE(
    d$home_run_rule$inside_park_counts
  )
...
validate_ruleset_config <- function(cfg) {
  ...
  b <- cfg$starting_count$balls
  s <- cfg$starting_count$strikes
  if (!.num_in_range(b, 0, 3)) {
    add("starting balls must be 0-3")
  }
  if (!.num_in_range(s, 0, 2)) {
    add("starting strikes must be 0-2")
  }
  if (!cfg$foul_out_rule %in% c("out", "one_courtesy_foul", "unlimited")) {
    add("invalid foul_out_rule")
  }

  bg <- cfg$batting_gender_rule$type
  valid_bg <- c(
    "none",
    "max_consecutive_males",
    "max_consecutive_same_gender",
    "min_females_per_n"
  )
  if (!bg %in% valid_bg) {
  ...
  if (
    !identical(bg, "none") &&
      !is.na(cfg$batting_gender_rule$n) &&
      cfg$batting_gender_rule$n < 1L
  ) {
    add("batting_gender_rule n must be >= 1")
  }

  if (!cfg$male_walk_rule %in% c("none", "two_bases_then_female")) {
    add("invalid male_walk_rule")
  }
  if (!.num_in_range(cfg$innings, 1, Inf)) {
    add("innings must be >= 1")
  }
  ...
  if (!cfg$batting_size_rule %in% c("max", "exact")) {
    add("batting_size_rule must be \"max\" or \"exact\"")
  }
  if (
    identical(cfg$batting_size_rule, "exact") &&
      length(bsz) == 1L &&
      is.na(bsz)
  ) {
    add("batting_size_rule \"exact\" requires a batting_size")
  }
  ...
  if (
    !cfg$home_run_rule$over_limit_result %in%
      c("out", "ground_rule_double", "single")
  ) {
    add("invalid over_limit_result")
  }
  if (
    !cfg$pinch_runner$eligibility %in%
...
next_batter_gender_ok <- function(cfg, prev_genders, next_gender) {
  ...
  if (identical(type, "max_consecutive_males")) {
    ...
    all_m <- length(recent) == n + 1L &&
      all(vapply(recent, is_g, logical(1), "M"))
    return(!all_m)
  }
  if (identical(type, "max_consecutive_same_gender")) {
    recent <- tail(c(prev_genders, next_gender), n + 1L)
    all_same <- length(recent) == n + 1L &&
      !is.na(recent[1]) &&
      length(unique(recent)) == 1L
    return(!all_same)
  }
  if (identical(type, "min_females_per_n")) {
    ...
    if (length(recent) < n) {
      return(TRUE)
    } # window not full yet; cannot be violated
...
.position_category <- function(pos) {
  ...
  if (!key %in% names(APP_CONFIG$POSITION_CATEGORY)) {
    return(NA_character_)
  }
...
evaluate_fielding <- function(cfg, defense_lineup) {
  ...
  if (length(fielders) == 0) {
    return(list())
  } # cannot evaluate without positions

  cat_of <- vapply(
    fielders,
    function(p) .position_category(p$position),
    character(1)
  )
  ...
  if (!is.na(minf) && Ftot < minf) {
    add(
      "min_females",
      sprintf("Need at least %d females in the field (have %d).", minf, Ftot)
    )
  }
  ...
  if (!is.na(maxm) && Mtot > maxm) {
    add(
      "max_males",
      sprintf("No more than %d males in the field (have %d).", maxm, Mtot)
    )
  }
  ...
  if (length(tiers) > 0) {
    ...
    req_of <- .as_int_or_na(tier$outfield)
    req_if <- .as_int_or_na(tier$infield)
    if (!is.na(req_of) && n_of < req_of) {
      add(
        "outfield_min",
        sprintf(
          "Need at least %d females in the outfield (have %d).",
          req_of,
          n_of
        )
      )
    ...
    if (!is.na(req_if) && n_if < req_if) {
      add(
        "infield_min",
        sprintf(
          "Need at least %d females in the infield (have %d).",
          req_if,
          n_if
        )
      )
    ...
    if (identical(tier$battery, "one")) {
      ppos <- Filter(
        function(p) identical(as.character(p$position), "P"),
        fielders
      )
      cpos <- Filter(
        function(p) identical(as.character(p$position), "C"),
        fielders
      )
      if (
        length(ppos) &&
          length(cpos) &&
          identical(ppos[[1]]$gender, cpos[[1]]$gender)
      ) {
...
game_should_end <- function(cfg, state) {
  ...
  if (.at_half_boundary(state)) {
    ...
    for (t in cfg$mercy_rule$tiers %||% list()) {
      ...
      if (!is.na(diff_needed) && completed >= after && diff >= diff_needed) {
        return(TRUE)
      }
    }
  }
