...
test_that("every outcome has a label, description, and valid category", {
  ...
  for (code in names(APP_CONFIG$outcome_meta)) {
    ...
    expect_true(
      nzchar(m$description %||% ""),
      info = paste(code, "needs a description")
    )
    expect_true(
      m$category %in% valid,
      info = paste(code, "has category", m$category)
    )
  }
})
...
test_that("the out categories match the reducer's out list", {
  outs <- names(Filter(
    function(m) identical(m$category, "out"),
...
test_that("the hit categories match the box score's hit list", {
  hits <- names(Filter(
    function(m) identical(m$category, "hit"),
    APP_CONFIG$outcome_meta
  ))
