...
has_code <- function(w, code) {
  any(vapply(w, function(x) identical(x$code, code), logical(1)))
}
...
test_that("warnings is a list of structured items and flags batting gender order", {
  ...
  s <- fold_events(list(
    new_event(
      ...
      list(
        ...
        away = list(team_id = "A", name = "A", lineup = lu)
      ),
      seq = 1L
    ),
    new_event(
  ...
  expect_true(all(vapply(
    s$warnings,
    function(x) x$severity %in% c("violation", "notice"),
    logical(1)
  )))
})
  ...
  s <- fold_events(list(
    new_event(
      ...
      list(
        ...
        away = list(team_id = "A", name = "A", lineup = away)
      ),
      seq = 1L
    ),
    new_event(
...
test_that("fielder_count is a maximum: short field is fine, oversized field warns", {
  cfg <- coerce_ruleset_config(list(fielding = list(fielder_count = 9L)))
  short <- list(
    make_player("h1", "H1", "M", 1L, 1L, "P"),
    make_player("h2", "H2", "F", 2L, 2L, "C")
  )
  s_short <- fold_events(list(new_event(
    "game_start",
    list(
      ruleset = cfg,
      first_bat = "away",
      home = list(team_id = "H", name = "Home", lineup = short),
      away = list(team_id = "A", name = "Away", lineup = short)
    ),
    seq = 1L
  )))
  expect_length(
    Filter(function(w) identical(w$code, "fielder_count"), s_short$warnings),
    0L
  )

  pos <- c("P", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF", "OF")
  big <- lapply(1:10, function(i) {
    make_player(paste0("h", i), paste0("H", i), "M", i, i, pos[i])
  })
  s_big <- fold_events(list(
    new_event(
      "game_start",
      list(
        ruleset = cfg,
        first_bat = "away",
        home = list(team_id = "H", name = "Home", lineup = big),
        away = list(team_id = "A", name = "Away", lineup = big)
      ),
      seq = 1L
    ),
    new_event(
  ...
  hit <- Filter(function(w) identical(w$code, "fielder_count"), s_big$warnings)
...
test_that("a lineup with no positions assigned raises no fielder-count notice", {
  ...
  start <- new_event(
    ...
    list(
      ...
      away = list(team_id = "A", name = "Away", lineup = lu)
    ),
    seq = 1L
  )
  s <- fold_events(list(start))
  expect_length(
    Filter(function(w) identical(w$code, "fielder_count"), s$warnings),
    0L
  )
})
