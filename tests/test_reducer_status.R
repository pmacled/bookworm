...
hr_evt <- function(runs, seq) {
  new_event("half_runs", list(runs = as.integer(runs)), seq = seq)
}

mercy_after_3 <- function() {
  coerce_ruleset_config(list(
    innings = 7L,
    mercy_rule = list(tiers = list(list(after_inning = 3L, differential = 20L)))
  ))
}
...
mercy_run <- function() {
  cfg <- mercy_after_3()
  c(
    list(start_runonly(cfg)),
    lapply(2:7, function(i) hr_evt(if (i == 2L) 20L else 0L, i))
  )
}
...
test_that("mercy does not fire in the top of the 3rd for a 3-inning tier", {
  ...
  expect_equal(s$inning, 3L)
  expect_equal(s$half, "top")
...
test_that("mercy fires at the end of the bottom of the 3rd", {
  ...
  expect_equal(s$inning, 4L)
  expect_equal(s$half, "top") # three innings complete
  expect_equal(s$status, "final")
  expect_true(any(vapply(
    s$warnings,
    function(w) identical(w$code, "final"),
    logical(1)
  )))
})
...
test_that("a shrinking differential un-finals the game", {
  ...
  cfg <- coerce_ruleset_config(list(
    innings = 7L,
    mercy_rule = list(tiers = list(list(after_inning = 1L, differential = 5L)))
  ))
...
test_that("mercy is not evaluated in the middle of a half-inning", {
  cfg <- coerce_ruleset_config(list(
    innings = 7L,
    mercy_rule = list(tiers = list(list(after_inning = 1L, differential = 2L)))
  ))
  start <- new_event(
    "game_start",
    list(
  ...
  hr <- function(id, seq) {
    new_event(
      ...
      list(
        ...
        rbi = 1L,
        outs_on_play = 0L,
        advances = list(make_advance(id, 0L, 4L, scored = TRUE))
      ),
      seq = seq
    )
  }
  ...
  base <- list(
    start,
    new_event("inning_end", list(), seq = 2L),
    new_event("inning_end", list(), seq = 3L)
  )
  mid <- fold_events(c(base, list(hr("a1", 4L), hr("a2", 5L))))
  expect_equal(mid$inning, 2L)
  expect_equal(mid$half, "top")
