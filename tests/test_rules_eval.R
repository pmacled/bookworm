...
test_that("no_two_males_consecutive flags a male after a male", {
  ...
  expect_false(next_batter_gender_ok(
    cfg,
    prev_genders = c("M"),
    next_gender = "M"
  ))
  expect_true(next_batter_gender_ok(
    cfg,
    prev_genders = c("M"),
    next_gender = "F"
  ))
...
test_that("next_batter_gender_ok fails open when the rule needs n but never got one", {
  ...
  cfg <- coerce_ruleset_config(list(
    batting_gender_rule = list(type = "max_consecutive_males")
  ))
...
test_that("next_batter_gender_ok is NA-safe: an unknown gender never satisfies the rule", {
  ...
  expect_true(next_batter_gender_ok(
    cfg,
    prev_genders = c(NA_character_),
    next_gender = "M"
  ))
  expect_true(next_batter_gender_ok(
    cfg,
    prev_genders = c("M"),
    next_gender = NA_character_
  ))

  cfg2 <- coerce_ruleset_config(list(
    batting_gender_rule = list(type = "min_females_per_n", n = 2L)
  ))
  expect_false(next_batter_gender_ok(
    cfg2,
    prev_genders = c(NA_character_),
...
gender_cfg <- function(type, n) {
  coerce_ruleset_config(
...
test_that("run cap limits non-open innings", {
  ...
  expect_equal(
    apply_run_cap(cfg, runs_before = 0L, runs_on_play = 8L, inning = 3L)$runs,
    5L
  )
  expect_equal(
    apply_run_cap(cfg, runs_before = 0L, runs_on_play = 8L, inning = 7L)$runs,
    8L
  )
})
...
test_that("the clamp branch passes an at-or-below-cap entry through unchanged", {
  ...
  cfg <- coerce_ruleset_config(list(
    run_cap = list(per_inning = 5L, same_play_runs_count = FALSE)
  ))
...
test_that("reducer surfaces a gender-order warning for the batter due up", {
  ...
  lineup <- list(
    make_player("m1", "M1", "M", 1L, 1L, 6L),
    make_player("m2", "M2", "M", 2L, 2L, 4L)
  )
  start <- new_event(
    "game_start",
    list(
      ...
      away = list(team_id = "A", name = "Away", lineup = lineup)
    ),
    seq = 1L
  )
  pa1 <- new_event(
  ...
  expect_true(any(vapply(
    s$warnings,
    function(x) identical(x$code, "batting_gender"),
    logical(1)
  )))
})
...
test_that("game_should_end does not crash on a mercy tier missing after_inning entirely", {
  ...
  st <- list(
    ...
    ruleset = cfg,
    outs = 0L
  ) # one completed inning; after_inning defaults to 1
...
st_at <- function(cfg, inning, home, away) {
  list(
    inning = inning,
    half = "top",
    score = list(home = home, away = away),
    ruleset = cfg,
    outs = 0L,
    runs_this_half = 0L
  )
...
test_that("regulation completion is not gated on the half-inning boundary", {
  ...
  cfg <- coerce_ruleset_config(list(innings = 7L))
  st <- st_at(cfg, 8L, 4L, 1L)
  st$outs <- 2L
  st$runs_this_half <- 3L
  expect_true(game_should_end(cfg, st))
})
