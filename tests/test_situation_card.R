...
st <- function() {
  ...
  s$lineups$home <- mk("h")
  s$teams <- list(
    away = list(team_id = "A", name = "Otters"),
...
test_that("the half-inning is an arrow, not the words top and bottom", {
  html <- as.character(situation_card_ui(st()))
  expect_true(
    grepl("&#9650;", html, fixed = TRUE) || grepl("\u25b2", html, fixed = TRUE)
  )
...
test_that("batter_line_text summarises hits, RBI, and walks", {
  expect_equal(
    batter_line_text(list(AB = 2L, H = 1L, RBI = 1L, BB = 0L, K = 0L, R = 0L)),
    "1-for-2, RBI"
  )
  expect_equal(
    batter_line_text(list(AB = 1L, H = 0L, RBI = 0L, BB = 0L, K = 1L, R = 0L)),
    "0-for-1, K"
  )
  expect_equal(
    batter_line_text(list(AB = 0L, H = 0L, RBI = 0L, BB = 1L, K = 0L, R = 0L)),
    "0-for-0, BB"
  )
  expect_equal(
    batter_line_text(list(AB = 0L, H = 0L, RBI = 0L, BB = 0L, K = 0L, R = 0L)),
    "first at-bat"
  )
})
