...
n_heavy <- function(frag) {
  lengths(regmatches(frag, gregexpr('stroke-width="3"', frag)))
}
...
test_that("the outcome and fielding notation are rendered", {
  frag <- scorebook_cell_svg(
    mk_path(0L, outcome = "GO", fielding = "6-3"),
    0,
    0,
    64
  )
...
test_that("the out number renders as a circled numeral", {
  frag <- scorebook_cell_svg(
    mk_path(0L, out_number = 2L, outcome = "K"),
    0,
    0,
    64
  )
...
test_that("RBI dots are drawn, one per RBI, capped at four", {
  ...
  expect_equal(
    n_dots(scorebook_cell_svg(mk_path(4L, scored = TRUE, rbi = 2L), 0, 0, 64)),
    2L
  )
  expect_equal(
    n_dots(scorebook_cell_svg(mk_path(4L, scored = TRUE, rbi = 9L), 0, 0, 64)),
    4L
  )
...
test_that("an out on the basepaths marks the base", {
  frag <- scorebook_cell_svg(
    mk_path(2L, out_at = 2L, out_number = 1L),
    0,
    0,
    64
  )
...
test_that("a batter out at the plate has no base mark", {
  frag <- scorebook_cell_svg(
    mk_path(0L, out_at = 0L, out_number = 1L, outcome = "K"),
    0,
    0,
    64
  )
...
mk_lineup <- function(prefix, n = 3L) {
...
test_that("the grid renders an svg with a row label per batter", {
  ...
  for (i in 1:3) {
    expect_true(grepl(paste("A", i), html, fixed = TRUE))
  }
})
...
test_that("the current batter's row label is bold", {
  st <- state_with(list())
  html <- as.character(render_scorebook_svg(
    st,
    "away",
    current_batter_id = "a2"
  ))
  expect_true(
    grepl("font-weight=\"700\"", html, fixed = TRUE) ||
...
test_that("batting around widens the inning into sub-columns", {
  st <- state_with(list(
    ...
    PA(
      "a2",
      "1B",
      list(Adv("a1", 1L, 2L), Adv("a2", 0L, 1L)),
      reached = 1L,
      seq = 3L
    ),
    PA(
      "a3",
      "1B",
      list(Adv("a1", 2L, 3L), Adv("a2", 1L, 2L), Adv("a3", 0L, 1L)),
      reached = 1L,
      seq = 4L
    ),
  ...
  html <- as.character(render_scorebook_svg(st, "away"))
  narrow <- as.character(render_scorebook_svg(
    state_with(list(
