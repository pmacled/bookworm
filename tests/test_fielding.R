...
fld <- function(cfg_fielding) {
  c <- default_ruleset_config()
  c$fielding <- cfg_fielding
  c
}
pl <- function(g, pos) {
  make_player(paste(g, pos), paste(g, pos), g, position = pos)
}
...
test_that("standard coed: a legal 4-female defense passes", {
  ...
  d <- list(
    pl("F", "P"),
    pl("M", "C"), # battery opposite (one F)
    pl("F", "SS"),
    pl("M", "1B"),
    pl("M", "2B"),
    pl("M", "3B"), # infield has 1 F
    pl("F", "LF"),
    pl("M", "LCF"),
    pl("M", "RCF"),
    pl("F", "RF") # outfield has 2 F
  ) # F total = 4, M total = 6
...
test_that("tier at 5 females requires 2 outfield + 2 infield", {
  ...
  d <- list(
    pl("F", "P"),
    pl("F", "C"), # battery both F (also flags battery_opposite; unchecked)
    pl("F", "SS"),
    pl("F", "1B"),
    pl("M", "2B"),
    pl("M", "3B"), # infield F=2
    ...
    pl("M", "LCF"),
    pl("M", "RCF"),
    pl("M", "RF") # outfield F=1  (needs 2 at 5F)
