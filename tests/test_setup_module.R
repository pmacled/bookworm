...
for (f in c("app_config.R","rules_engine.R","game_events.R","game_reducer.R",
            "rule_presets.R","lineup_validation.R","setup_module.R"))
...
test_that("setup_server produces a game_start with a run-only home team", {
  ...
  testServer(setup_server, {
    ...
    session$setInputs(away_name_1 = "Sam", away_gender_1 = "F", away_jersey_1 = 9, away_pos_1 = "SS",
      ...
      run_cap = 0, fielding_preset = "none",
      mercy_after_1 = 3, mercy_diff_1 = 0,
      mercy_after_2 = 4, mercy_diff_2 = 12,
      mercy_after_3 = 5, mercy_diff_3 = 0,
      start = 1)
    gs <- session$returned()
    ...
    expect_equal(gs$payload$ruleset$mercy_rule$tiers,
...
test_that("collect_ruleset coerces batting_size from the select string", {
  base_in <- list(fielding_preset = "none", start_balls = 1, start_strikes = 1,
    foul_out = "out", gender_rule = "none", gender_n = 2, innings = 7, run_cap = 0)
...
start_with_inputs <- function(extra) {
  ...
  testServer(setup_server, {
    ...
    args <- list(away_name_1 = "Sam", away_gender_1 = "F", away_jersey_1 = 9,
                 away_pos_1 = "SS", away_name = "Away", home_name = "Home",
                 start_balls = 0, start_strikes = 0, foul_out = "unlimited",
                 batting_size = "0", gender_rule = "none", innings = 7,
                 fielder_count = 0, run_cap = 0, fielding_preset = "none",
                 mercy_after_1 = 3, mercy_diff_1 = 0, mercy_after_2 = 4,
                 mercy_diff_2 = 0, mercy_after_3 = 5, mercy_diff_3 = 0)
...
test_that("a cleared N on a gender rule reports the reason instead of killing the session", {
  ...
  r <- start_with_inputs(list(gender_rule = "max_consecutive_males", gender_n = NA))
  expect_null(r$event)
  expect_true(any(grepl("max_consecutive_males batting rule requires n", r$notices, fixed = TRUE)),
...
test_that("a cleared Innings / balls / strikes box reports the reason too", {
  ...
  cases <- list(
    ...
    list(inputs = list(start_strikes = NA), want = "starting strikes must be 0-2"))
  for (case in cases) {
    r <- start_with_inputs(case$inputs)
    expect_null(r$event, info = case$want)
    expect_true(any(grepl(case$want, r$notices, fixed = TRUE)),
...
test_that("a cleared fielding-gender box cannot produce an unloadable game_start", {
  ...
  cfg <- collect_ruleset(list(
    fielding_preset = "custom", min_females = NA, max_males = NA,
    of_females = NA, if_females = NA, battery_mode = "any", fielder_count = NA,
    start_balls = 0, start_strikes = 0, foul_out = "unlimited", batting_size = "0",
    gender_rule = "none", innings = 7, run_cap = 0))
  expect_true(validate_ruleset_config(cfg)$ok)

  lineup <- list(make_player("a1","A1","F",1L,1L,"P"), make_player("a2","A2","M",2L,2L,"C"))
  evt <- build_game_start_event(cfg,
    list(team_id = "H", name = "Home", lineup = lineup),
    list(team_id = "A", name = "Away", lineup = lineup), "away")
...
test_that("a zeroed fielding-gender box still means zero, not unlimited-by-NA", {
  cfg <- collect_ruleset(list(
    fielding_preset = "custom", min_females = 0, max_males = 0,
    of_females = 0, if_females = 0, battery_mode = "any", fielder_count = 0,
    start_balls = 0, start_strikes = 0, foul_out = "unlimited", batting_size = "0",
    gender_rule = "none", innings = 7, run_cap = 0))
...
preset_inputs <- function(id) {
  ...
  ins <- list(
    preset = id,
    start_balls = cfg$starting_count$balls, start_strikes = cfg$starting_count$strikes,
    foul_out = cfg$foul_out_rule,
    batting_size = as.character(.na0(cfg$batting_size)),
    gender_rule = cfg$batting_gender_rule$type,
    gender_n = if (is.na(cfg$batting_gender_rule$n)) 1L else cfg$batting_gender_rule$n,
    innings = cfg$innings,
    ...
    fielding_preset = if (length(cfg$fielding$tiers)) "standard_coed" else "none")
  for (i in 1:3) {
    tiers <- cfg$mercy_rule$tiers
    t <- if (i <= length(tiers)) tiers[[i]] else NULL
    ins[[paste0("mercy_after_", i)]] <- if (is.null(t)) c(3L, 4L, 5L)[i] else t$after_inning
...
test_that("editing any exposed control flips the label to custom", {
  edits <- list(
    innings = 5, start_balls = 1, start_strikes = 1, foul_out = "out",
    batting_size = "10", gender_rule = "max_consecutive_same_gender",
    run_cap = 5, cap_same_play = FALSE, cap_ends_half = FALSE, open_last = FALSE,
    fielder_count = 11, hr_limit = 2, hr_limit_m = 1, hr_over = "single",
    hr_itp_counts = TRUE, pr_inning = 2, pr_game = 4, pr_player = 1,
    pr_elig = "last_out", pr_for = "pitcher_catcher", mercy_diff_1 = 20,
    fielding_preset = "standard_coed")
  base <- preset_inputs("standard_baseball")
  for (k in names(edits)) {
    ins <- base; ins[[k]] <- edits[[k]]
    expect_equal(collect_ruleset(ins)$preset, "custom", info = k)
  }
})

test_that("an unknown preset id is preserved rather than mislabelled", {
  ins <- preset_inputs("standard_baseball"); ins$preset <- "some_saved_league_ruleset"
...
test_that("collect_ruleset never throws, however empty or malformed the inputs", {
  ...
  expect_no_error(collect_ruleset(list(
    innings = NA, start_balls = NA, start_strikes = NA, run_cap = NA,
    gender_rule = "min_females_per_n", gender_n = NA, batting_size = NA,
    fielding_preset = "custom", min_females = NA, foul_out = NA_character_)))
})
