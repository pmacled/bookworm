...
p <- function(id, gender = "M", pos = "SS") {
  make_player(id, id, gender, 1L, 1L, pos)
}
...
test_that("max_per_inning counts only this inning and half", {
  ...
  expect_equal(
    msgs(r),
    "Only 1 pinch runner(s) allowed per inning; 1 already used."
  )

  used_other <- list(list(
    inning = 2L,
    half = "top",
    team = "away",
    out_player_id = "x",
  ...
  expect_true(
    evaluate_pinch_runner(cfg, base_state(used_other), p("r1"), p("r2"))$ok
  )
  ...
  expect_true(
    evaluate_pinch_runner(cfg, base_state(used_other_half), p("r1"), p("r2"))$ok
  )
})

test_that("max_per_game counts every inning for this team", {
  cfg <- pr_cfg(max_per_game = 2L)
  used <- lapply(1:2, function(i) {
    list(
      inning = i,
      half = "top",
      team = "away",
  ...
  expect_equal(
    msgs(r),
    "Only 2 pinch runner(s) allowed per game; 2 already used."
  )
  ...
  used_other_team <- lapply(1:2, function(i) {
    list(
      inning = i,
      half = "top",
      team = "home",
  ...
  expect_true(
    evaluate_pinch_runner(cfg, base_state(used_other_team), p("r1"), p("r2"))$ok
  )
})

test_that("max_per_game = 0 forbids pinch runners entirely, with its own code", {
  r <- evaluate_pinch_runner(
    pr_cfg(max_per_game = 0L),
    base_state(),
    p("r1"),
    p("r2")
  )
...
test_that("same_gender eligibility", {
  ...
  expect_true(
    evaluate_pinch_runner(cfg, base_state(), p("r1", "F"), p("r2", "F"))$ok
  )
  r <- evaluate_pinch_runner(cfg, base_state(), p("r1", "F"), p("r2", "M"))
  expect_false(r$ok)
...
test_that("allowed_for = pitcher_catcher restricts who may be run for", {
  ...
  expect_true(
    evaluate_pinch_runner(cfg, base_state(), p("r1", "M", "P"), p("r2"))$ok
  )
  expect_true(
    evaluate_pinch_runner(cfg, base_state(), p("r1", "M", "C"), p("r2"))$ok
  )
